...
(BE / "runtime" / "browser_supervisor.py").write_text('''"""Patchright-based browser supervisor.
...
print("  [OK] backend/app/runtime/browser_supervisor.py")

# ── unit tests (no real browser) ──
(ROOT / "tests" / "test_browser_supervisor.py").write_text('''import os
...
''', encoding="utf-8", newline="\n")
print("  [OK] tests/test_browser_supervisor.py")

# ── syntax + tests ──
import ast
for f in ["backend/app/runtime/browser_supervisor.py", "tests/test_browser_supervisor.py"]:
...
print("\n==> running browser supervisor tests")
r = subprocess.run([str(PY), "-m", "pytest", "-q",
                    "tests/test_browser_supervisor.py", "-o", "asyncio_mode=auto"],
...
git(["add","-A"])
r = git(["commit","-m","feat(runtime): browser supervisor with Patchright Chrome + tabs (Step 3)"])
print((r.stdout.strip() or r.stderr.strip())[:200])

print()
print("=" * 60)
print("STEP 3 COMPLETE — supervisor class + 7 unit tests")
print("=" * 60)
