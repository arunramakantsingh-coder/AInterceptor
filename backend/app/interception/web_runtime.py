...
class WebNetworkCapture:
    ...
    def _matches(self, response: Any) -> bool:
        ...
        return not self.spec.response_content_types or any(marker in content_type for marker in self.spec.response_content_types)
    ...
    def visit(node: Any) -> None:
        if isinstance(node, dict):
            for k, v in node.items():
                if isinstance(v, str) and v.strip() and str(k).lower() in {"content", "text", "completion", "response", "answer", "message"}:
                    candidates.append(v)
                else:
                    visit(v)
...
def _gemini_json_candidates(text: str) -> list[Any]:
    text = text.lstrip()
    if text.startswith(")]}'"):
        text = text.split("\n", 1)[1] if "\n" in text else ""
    objects: list[Any] = []
    ...
    while pos < len(text):
        ...
        pos = start + length
        try:
            objects.append(json.loads(frame))
        except json.JSONDecodeError:
            continue
    if objects:
        return objects
...
class BrowserWebRuntime(ProviderRuntime):
    ...
    async def _execute_cdp(self, request: ProviderExecutionRequest) -> AsyncIterator[StreamEvent]:
        ...
        self._cdp.on("Network.dataReceived", on_data)
        self._cdp.on("Network.loadingFinished", on_finished)
        ...
        try:
            ...
            if response_status is None or response_status >= 400:
                yield StreamEvent(self.provider, request.request_id, EventType.STREAM_FAILED, sequence, metadata={"status": response_status, "url": response_url})
                return
            ...
            for event_name, handler in (("Network.requestWillBeSent", on_request), ("Network.responseReceived", on_response), ("Network.dataReceived", on_data), ("Network.loadingFinished", on_finished), ("Network.loadingFailed", on_failed)):
                try:
                    self._cdp.remove_listener(event_name, handler)
                except Exception:
                    pass
    ...
    async def execute(self, request: ProviderExecutionRequest) -> AsyncIterator[StreamEvent]:
        ...
        async with self._lock:
            if self.cdp_url and self._cdp is not None:
                async for event in self._execute_cdp(request):
                    yield event
                return
