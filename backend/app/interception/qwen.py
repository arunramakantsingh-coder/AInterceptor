"""Qwen Web runtime using browser transport interception."""
...
def parse_qwen_web(body: str) -> str:
...
class QwenRuntime(NonClaudeWebRuntime):
    provider = "qwen"
    ...
    def __init__(self, session_path: str | None = None, headless: bool = False, cdp_url: str | None = None):
        super().__init__(
            ...
            session_path=session_path or os.getenv("AINTERCEPTOR_QWEN_STORAGE_STATE") or str(Path(".ainterceptor") / "qwen" / "storage_state.json"),
            cdp_url=cdp_url or os.getenv("AINTERCEPTOR_QWEN_CDP_URL") or existing_chrome_cdp(),
            headless=headless,
            parser=parse_qwen_web,
        )
