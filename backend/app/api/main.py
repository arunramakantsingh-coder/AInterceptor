...
SESSION_PATH = pathlib.Path(os.getenv('AINTERCEPTOR_CLAUDE_SESSION_PATH', str(REPO_ROOT / '.ainterceptor' / 'claude' / 'storage_state.json')))
app = FastAPI(title='AIRouter Gateway API', version='0.3.0')
...
async def chat_completions(req: ChatRequest, _: None = Depends(_require_auth)):
    ...
    if not req.stream:
        ...
        return {'id':f'airouter-{uuid.uuid4().hex}','object':'chat.completion','created':int(time.time()),'model':req.model,'choices':[{'index':0,'message':{'role':'assistant','content':result['content']},'finish_reason':result['finish_reason']}],'usage':{'prompt_tokens':0,'completion_tokens':0,'total_tokens':0}}
