    ...
    if rows:
        ...
        for r in rows:
            row = (
                ...
                + ('<span style="color:#4ade80;">&#9679; ' + W.esc(r.status) + '</span>'
                   if r.status == "active" else W.esc(r.status)) + '</td>'
                ...
                '<td style="padding:12px 8px;border-bottom:1px solid #1f1f1f;color:#888;font-size:12px;">'
                + _fmt_age(r.last_used_at) + '</td>'
                '<td style="padding:12px 8px;border-bottom:1px solid #1f1f1f;text-align:right;white-space:nowrap;">'
                '<a class="btn btn-secondary" href="/dashboard/agents" style="padding:5px 12px;font-size:12px;margin-right:6px;">Refresh</a>'
                '<form method="POST" action="/dashboard/sessions/' + W.esc(r.provider) + '/delete" '
                'style="display:inline" onsubmit="return confirm('Delete ' + W.esc(r.provider) + ' session? You will need to log in again.')">'
                '<button type="submit" class="btn-secondary" style="padding:5px 12px;font-size:12px;color:#f87171;">Delete</button>'
                '</form>'
                '</td>'
                '</tr>'
            )
        ...
        table = ('<p class="muted">No sessions yet. '
                 'Visit <a href="/dashboard/agents">Agents</a> to connect a provider.</p>')

    body = W.dashboard_nav("/dashboard/sessions") + f'''
<div class="container">
  <div class="row" style="justify-content:space-between;">
    <div><h2>Provider sessions</h2>
      <p class="muted">Each row is a logged-in provider stored for your account.
      Refresh opens the Agents page where one click re-runs the login.</p></div>
    <a class="btn btn-secondary" href="/dashboard">← back</a>
  </div>
...
def dashboard_sessions_delete(provider: str,
                              user: User = Depends(current_user_web),
                              db: Session = Depends(get_db)):
